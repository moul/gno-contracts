// The chain half: one JSON-RPC abci_query, standard library only.
//
// Adapted from tools/gnohome/chain.go, which is the same protocol against a
// different manifest. Copied rather than shared because tools/ has no common
// package and each tool is a self-contained main: the two manifests have
// different shapes, so the parser is the only part that would not have been
// shared anyway.
package main
...
// remotePost is one line of the realm's Manifest(): what the chain holds for
// one post, without the body.
...
// round trip for the whole diff: the manifest carries a hash per post covering
// title, date, tags and body together, so no body is ever downloaded just to
// find out it has not changed.
//
...
func fetchManifest(remote, realm string) (map[string]remotePost, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "package not found") ||
			...
			return map[string]remotePost{}, nil
		}
		return nil, err
	}
...
func parseManifest(raw string) (map[string]remotePost, error) {
	out := map[string]remotePost{}
	for _, line := range strings.Split(raw, "\n") {
		...
		if len(f) != 5 {
			return nil, fmt.Errorf("manifest line has %d fields, want 5: %q", len(f), line)
		}
		...
		out[f[0]] = remotePost{rev: rev, date: f[2], size: size, hash: f[4]}
	}
	return out, nil
}
