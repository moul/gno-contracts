...
// printBatch writes the document and the two commands that act on it. This
// tool holds no key and broadcasts nothing: it prints, and the human signs.
func printBatch(out *os.File, cfg config, changes []change, opt txOptions, path string) error {
	...
	// it with "no signers". Signing gates the broadcast so a bad paste fails
	// on disk instead of at the chain.
	...
	sh := fmt.Sprintf("#!/bin/sh\n# generated by gnoblog tx; review, then: sh %s\n"+
