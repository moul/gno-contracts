...
func printBatch(out *os.File, cfg config, changes []change, opt txOptions, path string) error {
	...
	// Chained with &&, deliberately. `gnokey broadcast` does NOT refuse an
	// unsigned document locally: it sends it and lets the ante handler reject
	// it with "no signers", which is a confusing round trip to the chain for
	// a mistake visible on disk. Two separate lines invite exactly that when a
	// paste goes wrong, so signing gates the broadcast.
	fmt.Fprintf(out, "gnokey sign -tx-path %s \\\n"+
		"  -chainid %s -account-number %s -account-sequence %s \\\n  %s \\\n"+
		"&& gnokey broadcast -remote %s %s\n",
		shellQuote(path), cfg.chainID, acct.Number, acct.Sequence, cfg.key,
		cfg.remote, shellQuote(path))

	// And write it next to the document, so there is nothing to paste at all.
	script := strings.TrimSuffix(path, ".json") + ".sh"
	sh := fmt.Sprintf("#!/bin/sh\n# generated by gnohome tx -batch; review, then: sh %s\n"+
		"# Valid only while %s is at sequence %s.\nset -e\n\n"+
		"gnokey sign -tx-path %s \\\n  -chainid %s -account-number %s -account-sequence %s \\\n  %s\n"+
		"gnokey broadcast -remote %s %s\n",
		script, cfg.key, acct.Sequence,
		shellQuote(path), cfg.chainID, acct.Number, acct.Sequence, cfg.key,
		cfg.remote, shellQuote(path))
