...
`tx` builds one unsigned transaction document and hands it to `gnokey`, which
signs and broadcasts it. Several posts go in one document, so publishing three
is one passphrase prompt and one atomic broadcast. After a redeploy, `tx -all`.

`status` is the review step. `tx -print` writes the commands out and runs
nothing, for the case where the signing happens somewhere else.
